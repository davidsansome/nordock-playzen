...
void main()
{
    ...
    // Disabled by Robin - Jun 05.  Too many people were exploiting it
    /*if (GetStringLeft(sItemTag, 8) == "dmfi_pc_")
    {
        if (sItemTag == "dmfi_pc_follow")
        {
            if (GetTag(GetArea(oUser)) == "FuguePlane")
            {
                SendMessageToAllDMs(GetName(oUser) + " (" + GetPCPlayerName(oUser) + ") tried to use the Auto Follow widget in the Fugue Plane.  He/she may be exploiting.");
                SendMessageToPC(oUser, "Using the Auto Follow widget in the Fugue Plane is forbidden.  A DM has been alerted, and further action will be taken if you continue.");
                return;
            }
...
// added logging line
            WriteTimestampedLogEntry(GetName(oUser)+ " is attempting to use a DM item.");
            return;
        }
