...
void RespawnTree(location lLoc)
    ...
    CreateObject(OBJECT_TYPE_PLACEABLE,"oaktree002",lLoc,TRUE);
    }
...
void main()
    {
        ...
        if (GetTag(oWeaponOnPlayer) ==  "ITEM_WOODAXE")  // checks for the correct tag
        {
           if (ichophit <= 10) // if the succeed roll is 10 or greater they get wood
           {
                ...
                {
                CreateItemOnObject("ats_r_celm_b_sta", GetLastKiller(), d6()+1);
                }
                ...
                {
                CreateItemOnObject("ats_r_cllm_b_sta", GetLastKiller(), d6()+1);
                }
