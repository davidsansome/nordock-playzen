...
void main()
{
    ...
    // Changes by Nakey - When leaving the area the hissing bog gate is unlocked
    // so players can exit.
    SetLocked(GetObjectByTag("HissingBogGate"), 0);
    AssignCommand(GetObjectByTag("HissingBogGate"), ActionLockObject(OBJECT_SELF));
}
