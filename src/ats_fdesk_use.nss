...
void main()
{
    ...
    if(ATS_GetTradeskill(oPlayer, CSTR_SKILLNAME_FLETCHING) <= 0)
        FloatingTextStringOnCreature("You do not know how to use this.", oPlayer, FALSE);
    // Check to make sure the proper tool is equipped
    else if(ATS_GetToolForCraft(oPlayer, CSTR_SKILLNAME_FLETCHING) == OBJECT_INVALID)
    ...
    else
    {
        ...
        ClearAllActions();
        ATS_SetCurrentCraftType(oPlayer, ATS_CRAFT_TYPE_FLETCHING);
