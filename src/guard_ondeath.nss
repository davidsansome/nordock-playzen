...
//Shout Attack my target, only works with the On Spawn In setup
    ...
    // For the WP spawn system
    if (GetLocalInt(OBJECT_SELF, "WaypointSpawnedCreature") == 1)
    {
        object oWaypoint = GetLocalObject(OBJECT_SELF, "CreatorWaypoint");
        if (GetObjectType(oWaypoint) != OBJECT_TYPE_WAYPOINT)
            return;

        SetLocalInt(oWaypoint, "RespawnDisabled", 1);
        DelayCommand(20.0f * 60.0f, SetLocalInt(oWaypoint, "RespawnDisabled", 0));
    }
