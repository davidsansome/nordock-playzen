...
//Shout Attack my target, only works with the On Spawn In setup
    ...
    {
        int nType = GetRacialType(OBJECT_SELF);
        if( (nType == RACIAL_TYPE_ANIMAL ||
             nType == RACIAL_TYPE_BEAST ||
             nType == RACIAL_TYPE_VERMIN
            ) &&
            OBJECT_INVALID == GetFirstItemInInventory())
        {
            SetIsDestroyable(FALSE,FALSE,FALSE);
            DelayCommand(60.0, DestroyCreature(OBJECT_SELF));
        }
        else
        {
            // Remove polymorph effects before leaving a corpse
            // This fixes an exploit that would make droppable (planes) weapons
            // always drop from a creature that was polymorphed at time of death
            int bWasPolymorphed = 0;
