...
/*
    ...
    Modified by Robin (May 05) to use SQL
...
const string strJournalDatabaseName = "nordock_journal";
...
void dhAddJournalQuestEntry(string strCategoryTag, int iEntryId, object oCreature,
        ...
        int bMarkAsFinished = FALSE, int bNotifyDM = TRUE)
{
    object oPlayer;
    ...
    if (bNotifyDM)
        SendMessageToAllDMs("Journal Update: " + GetName(oCreature) + " - " + strCategoryTag + " part " + IntToString(iEntryId));
    ...
    if (bMarkAsFinished) {
        ...
        SSetPersistentInt(oCreature, strTag, TRUE, 0, dhGetJournalDatabaseName(oCreature));
        strTag = GGetPersistentString(oCreature, "strCompletedQuests", dhGetJournalDatabaseName(oCreature));
        strTag += (strCategoryTag + ".");
        SSetPersistentString(oCreature, "strCompletedQuests", strTag, 0, dhGetJournalDatabaseName(oCreature));
    }

    // store the serialized string
    SSetPersistentString(oCreature, "strQuestStates", strSerialized, 0, dhGetJournalDatabaseName(oCreature));
}
        ...
        int bAllPlayers = FALSE, int bRemoveIfFinished = FALSE)
{
    ...
    if (bRemoveIfFinished) {
        ...
        iState = GGetPersistentInt(oCreature, strTag, dhGetJournalDatabaseName(oCreature));
        if (iState) {
            SSetPersistentInt(oCreature, strTag, FALSE, 0, dhGetJournalDatabaseName(oCreature));
        }
    }
...
int dhGetJournalQuestState(string strPlotId, object oCreature)
{
    ...
    int bFlag = GGetPersistentInt(oCreature, strTag, dhGetJournalDatabaseName(oCreature));
    if (bFlag) return -1;
...
void dhClearJournal(object oPlayer)
{
    ...
    strCompleted = GGetPersistentString(oPlayer, "strCompletedQuests", dhGetJournalDatabaseName(oPlayer));
    while (TRUE) {
        ...
        SSetPersistentInt(oPlayer, strTag, FALSE, 0, dhGetJournalDatabaseName(oPlayer));
    }

    // clear the pending quests and completed quests
    SSetPersistentString(oPlayer, "strCompletedQuests", "", 0, dhGetJournalDatabaseName(oPlayer));
    SSetPersistentString(oPlayer, "strQuestStates", "", 0, dhGetJournalDatabaseName(oPlayer));
}
...
void dhLoadJournalQuestStates(object oPlayer)
{
    ...
    strSerialized = GGetPersistentString(oPlayer, "strQuestStates", dhGetJournalDatabaseName(oPlayer));
    while (TRUE) {
        ...
        AddJournalQuestEntry(strToken1, StringToInt(strToken2), oPlayer, FALSE, FALSE, FALSE);
        strTag = "strQuestTag"+IntToString(iMaxIndex);
...
void dhHideJournalQuestEntry(object oPlayer, string strPlotId)
{
    ...
    strTag = "bQuestFinished_" + strPlotId;
    int bFlag = GGetPersistentInt(oPlayer, strTag, dhGetJournalDatabaseName(oPlayer));
    if (!bFlag) return;
    ...
    SSetPersistentString(oPlayer, "strQuestStates", strSerialized, 0, dhGetJournalDatabaseName(oPlayer));
