/****************************************************
  On Tinkering Toolset's Use Script
  ats_anvil_use
  ...
  ***Ambrosia Tradeskill System***
  ...
  This script should be placed on the custom tinkering toolset's
  OnUsed trigger.  This script checks to make sure
  the player has the tinkering skill and has the
  proper fletching tool.  If so, it starts the
  tinkering conversation menu.
***********************************************/
...
void main()
{
    ...
    if(ATS_GetTradeskill(oPlayer, CSTR_SKILLNAME_TINKER) <= 0)
        FloatingTextStringOnCreature("You do not know how to use this.", oPlayer, FALSE);
    // Check to make sure the proper tool is equipped
    else if(ATS_GetToolForCraft(oPlayer, CSTR_SKILLNAME_TINKER) == OBJECT_INVALID)
    ...
    else
    {
        ...
        ClearAllActions();
        ATS_SetCurrentCraftType(oPlayer, ATS_CRAFT_TYPE_TINKER);
