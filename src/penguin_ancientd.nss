...
// Created 7/11/2002
    ...
    // PENGUIN CODE
    if (GetLocalInt(OBJECT_SELF, "GongSpawned"))
    {
        // Reset local variables
        object oGong = GetObjectByTag("PenguinGong");
        DelayCommand(30.0, DeleteLocalInt(oGong, "Part1"));
        DelayCommand(30.0, DeleteLocalInt(oGong, "Part2"));

        // Open exit door
        object oDoor2 = GetObjectByTag("PenguinDoor2");
        AssignCommand(oGong, ActionDoCommand(SetLocked(oDoor2, FALSE)));
        AssignCommand(oGong, ActionOpenDoor(oDoor2));
    }
